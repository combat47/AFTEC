...
def test_mock_ingester_returns_valid_sample():
    """Test that mock ingester returns a valid SoilSample within ranges."""
...
def test_mock_ingester_randomness():
    """Test that consecutive samples are not all identical (probabilistic)."""
    ingester = MockIngester()
    samples = [ingester.read() for _ in range(10)]
    temps = [s.temperature_c for s in samples]
    phs = [s.ph for s in samples]
    # Should not be all identical (though could be by chance)
